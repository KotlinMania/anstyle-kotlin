 ...
 * provided to match against.
 */
fun ansiToRgb(color: AnsiColor, palette: Palette = Palette.DEFAULT): RgbColor = palette.rgbFromAnsi(color)
...
fun xtermToAnsi(color: Ansi256Color, palette: Palette = Palette.DEFAULT): AnsiColor =
    when (val index = color.index.toInt()) {
        0 -> AnsiColor.Black
        1 -> AnsiColor.Red
        2 -> AnsiColor.Green
        3 -> AnsiColor.Yellow
 ...
 * provided to match against.
 */
fun rgbToAnsi(color: RgbColor, palette: Palette = Palette.DEFAULT): AnsiColor = palette.findMatch(color)
 ...
 */
internal val XTERM_COLORS: Array<RgbColor> =
    arrayOf(
